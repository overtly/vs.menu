...
{
    public class Net4CodeGenerator : ICodeGenerator
    {
        //相关模板
        protected string TlClient = Encoding.UTF8.GetString(Resources.Client);

        protected string TlClientOperation = Encoding.UTF8.GetString(Resources.Client_Operation);

        protected string TlClientOperationVoid = Encoding.UTF8.GetString(Resources.Client_Operation_Void);

        protected string TlFaceClient = Encoding.UTF8.GetString(Resources.Face_Client);

        protected string TlFaceClientOperation = Encoding.UTF8.GetString(Resources.Face_Client_Operation);

        protected string TlAsyncService = Encoding.UTF8.GetString(Resources.AsyncService);

        protected string TlFaceServer = Encoding.UTF8.GetString(Resources.Face_Server);

        protected string TlFaceServerOperation = Encoding.UTF8.GetString(Resources.Face_Server_Operation);

        protected string TlProcessor = Encoding.UTF8.GetString(Resources.Processor);

        protected string TlProcessorProcess = Encoding.UTF8.GetString(Resources.Processor_Process);

        protected string TlProcessorProcessVoid = Encoding.UTF8.GetString(Resources.Processor_Process_Void);
            ...
            foreach (var childService in template.Services)
            {
                ...
                foreach (var childOp in childService.Operations)
                {
                    ...
                    strFaceOperations += TlFaceClientOperation
                        ...
                        .Replace("<%=summary%>", childOp.Summary); // yaofeng add 
